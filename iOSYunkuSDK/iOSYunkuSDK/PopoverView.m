...
#define _RGB(x,y,z)         [UIColor colorWithRed:x/255.0f green:y/255.0f blue:z/255.0f alpha:1]
...
#define BASE_BKG_COLOR                  _RGB(51, 56, 64)
...
@property(nonatomic,strong) NSArray *actBtns;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        self.borderColor = BASE_BKG_COLOR;
...
-(void)initBtns
{
    CGFloat startV=kArrowHeight+SPACE;
    for (NSInteger i=0; i<self.actBtns.count; ++i) {
        UIButton *btn=_actBtns[i];
        btn.frame=CGRectMake(0, startV+ROW_HEIGHT*i, btn.frame.size.width, ROW_HEIGHT);
        [self addSubview:btn];
        
        if (i!=self.actBtns.count-1) {
            UILabel *line=[[UILabel alloc] initWithFrame:CGRectMake(10, btn.frame.origin.y+btn.frame.size.height-0.5, btn.frame.size.width-20, 0.5)];
            [line setBackgroundColor:_RGB(80, 80, 80)];
...
-(CGRect)getViewFrame
{
    ...
    frame.size.height = [self.actBtns count] * ROW_HEIGHT + SPACE*2+kArrowHeight;
...
//    }
    ...
    CGRect rcWin=[UIScreen mainScreen].bounds;
    //右间隔最小5x
    if ((frame.origin.x + frame.size.width) > rcWin.size.width-5) {
        frame.origin.x = rcWin.size.width-5 - frame.size.width;
    }
...
-(void)show
{
    ...
    [UIView animateWithDuration:0.1f delay:0.f options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        [UIView animateWithDuration:0.04f delay:0.f options:UIViewAnimationOptionCurveEaseInOut animations:^{
...
-(UITableView *)tableView
{
    ...
    _tableView.backgroundColor = BASE_BKG_COLOR;
...
{
    static NSString *identifier = @"morepopviewcell";
...
//    }
    ...
    [cell addBorderLineInDirection:1 width:0.5 color:[UIColor grayColor]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.delegate) {
        [delegate selectRowOfPopview:indexPath.row];
    }
    [self dismiss:YES];
}
